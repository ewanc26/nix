...
#    1. Configures Samba (smbd) with the fruit / streams_xattr VFS stack that
#       macOS requires for reliable extended-attribute and resource-fork handling.
#    2. Advertises the share via Avahi (mDNS) so Macs discover it automatically
#       in System Settings -> General -> Time Machine.
#    3. Creates /srv/timemachine with correct ownership via systemd-tmpfiles.
#
...
#
#  Settings knobs (settings/config/server.nix -> timemachine):
...
let
  tm    = cfgLib.cfg.server.timemachine;
...
lib.mkIf tm.enable {
  ...
  services.samba = {
    enable       = true;
    openFirewall = false;   # we manage ports explicitly below
    ...
    settings = {
      ...
      ${tm.shareName} = {
        ...
        # Tell macOS this share is a Time Machine destination
