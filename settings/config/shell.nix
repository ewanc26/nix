{
  ...
  # ── Nix tool aliases — shared by both platforms ───────────────────────────
  ...
  linuxAliases = {
    # nixToolAliases are merged by zsh.nix — only put Linux-only entries here
    cleanup = "sudo nix-collect-garbage -d && nix-collect-garbage -d";
  };

  # macOS-specific aliases
  darwinAliases = {
    # nixToolAliases are merged by zsh.nix — only put macOS-only entries here
