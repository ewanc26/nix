...
check_secret() {
  ...
  # Check it's valid JSON (all sops files are JSON on disk)
  ...
  if [ $exit_code -eq 0 ]; then
    # Check if output looks like raw sops JSON (failed decrypt written as plaintext)
    if echo "$plaintext" | grep -q '"data": "ENC\['; then
      echo "  ✗ Decrypted to raw SOPS JSON — encrypted with wrong format"
